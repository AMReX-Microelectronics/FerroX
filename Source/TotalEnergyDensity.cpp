                ...
                const Geometry& geom)
{
        ...
        for ( MFIter mfi(P_old); mfi.isValid(); ++mfi )
        {
            ...
            amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                ...
                //      - g11 * DoubleDPDz(pOld, z, z_hi, z_lo, P_BC_flag_lo, P_BC_flag_hi, FE_lo, FE_hi, DE_hi, prob_lo, prob_hi,lambda, i, j,  k, dx)
                //      + DPhiDz(phi, z, z_hi, z_lo, prob_lo, prob_hi, Phi_Bc_hi, Phi_Bc_lo, DE_hi, i, j, k, dx)
                //     );
