...
function(create_sundials_aliases)
    ...
    if(BUILD_SHARED_LIBS)
        ...
        if(NOT TARGET SUNDIALS::nvecmpimanyvector)
    ...
    if(BUILD_SHARED_LIBS)
        if(FerroX_COMPUTE STREQUAL CUDA AND TARGET sundials_nveccuda_shared AND NOT TARGET SUNDIALS::nveccuda)
            add_library(SUNDIALS::nveccuda ALIAS sundials_nveccuda_shared)
            if(TARGET sundials_cvode_fused_cuda_shared AND NOT TARGET SUNDIALS::cvode_fused_cuda)
                add_library(SUNDIALS::cvode_fused_cuda ALIAS sundials_cvode_fused_cuda_shared)
            endif()
        elseif(FerroX_COMPUTE STREQUAL HIP AND TARGET sundials_nvechip_shared AND NOT TARGET SUNDIALS::nvechip)
            add_library(SUNDIALS::nvechip ALIAS sundials_nvechip_shared)
            if(TARGET sundials_cvode_fused_hip_shared AND NOT TARGET SUNDIALS::cvode_fused_hip)
                add_library(SUNDIALS::cvode_fused_hip ALIAS sundials_cvode_fused_hip_shared)
            endif()
        elseif(FerroX_COMPUTE STREQUAL OMP AND TARGET sundials_nvecopenmp_shared AND NOT TARGET SUNDIALS::nvecopenmp)
            add_library(SUNDIALS::nvecopenmp ALIAS sundials_nvecopenmp_shared)
        endif()
    else()
        if(FerroX_COMPUTE STREQUAL CUDA AND TARGET sundials_nveccuda_static AND NOT TARGET SUNDIALS::nveccuda)
            add_library(SUNDIALS::nveccuda ALIAS sundials_nveccuda_static)
            if(TARGET sundials_cvode_fused_cuda_static AND NOT TARGET SUNDIALS::cvode_fused_cuda)
                add_library(SUNDIALS::cvode_fused_cuda ALIAS sundials_cvode_fused_cuda_static)
            endif()
        elseif(FerroX_COMPUTE STREQUAL HIP AND TARGET sundials_nvechip_static AND NOT TARGET SUNDIALS::nvechip)
            add_library(SUNDIALS::nvechip ALIAS sundials_nvechip_static)
            if(TARGET sundials_cvode_fused_hip_static AND NOT TARGET SUNDIALS::cvode_fused_hip)
                add_library(SUNDIALS::cvode_fused_hip ALIAS sundials_cvode_fused_hip_static)
            endif()
        elseif(FerroX_COMPUTE STREQUAL OMP AND TARGET sundials_nvecopenmp_static AND NOT TARGET SUNDIALS::nvecopenmp)
